    ...
    try {
        ...
        if (toppings && toppings.length > 0) {
            for (const topping_id of toppings) {
                await pool.query("INSERT INTO ToppingStorages (topping_id, cart_item_id) VALUES (?, ?)", [
                    topping_id,
                    result.insertId,
                ]);
            }
        ...
        if (size_id || cartToppings.length > 0) {
            let total_topping_price = cartToppings.reduce((acc, curr) => acc + parseFloat(curr.topping_price), 0);
            ...
            cartItem = {
                ...
                order_item_price:
                    ...
                    parseFloat(cart[0].size_price) * parseFloat(cart[0].quantity),
            };
        } else {
            cartItem = {
                ...cart[0],
                toppings: null,
                order_item_price: parseFloat(cart[0].quantity) * parseFloat(cart[0].product_price),
            };
        }

        await pool.commit();

        return res.status(200).json({ status: 200, message: "success", data: cartItem });
    } catch (error) {
        await pool.rollback();
...
async function getUserCart(req, res) {
    ...
    try {
        ...
        for (const cart of carts) {
            ...
            if (cart.size_id || cartToppings.length > 0) {
                ...
                cartItem = {
                    ...
                    order_item_price:
                        (total_topping_price + parseFloat(cart.product_price) + parseFloat(cart.size_price)) *
                ...
                cartItem = {
                    ...cart,
                    toppings: null,
                    order_item_price: parseFloat(cart.quantity) * parseFloat(cart.product_price),
                };
            }
        ...
        return res.status(200).json({ status: 200, message: "success", data: userCart });
    } catch (error) {
        await pool.rollback();
...
async function editCart(req, res) {
    ...
    try {
        ...
        await pool.query("UPDATE CartItems SET size_id = ?, quantity = ? WHERE id = ?", [
            size_id || null,
            quantity,
            cart_item_id,
        ]);
        ...
        if (toppings && toppings.length > 0) {
            if (toppings.length < toppingCount) {
                await pool.query("DELETE FROM ToppingStorages WHERE cart_item_id = ? AND topping_id NOT IN (?)", [
                    cart_item_id,
                    toppings,
                ]);
            } else {
                ...
                const newToppings = toppings.filter((topping) => !existingToppings.includes(topping));

                for (const topping of newToppings) {
                    await pool.query("INSERT INTO ToppingStorages (cart_item_id, topping_id) VALUES (?, ?)", [
                        cart_item_id,
                        topping,
                    ]);
                }
            ...
            await pool.query("DELETE FROM ToppingStorages WHERE cart_item_id = ?", [cart_item_id]);
        }
...
async function getTotalItemUserCart(req, res) {
    ...
    try {
        ...
        return res.status(200).json({ status: 200, message: "success", data: result[0].count });
    } catch (error) {
        return res.status(500).json({ status: 500, message: error.message });
    } finally {
        if (pool) await pool.end();
